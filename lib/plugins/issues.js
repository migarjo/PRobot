...
module.exports = class Issues extends Plugin {
  ...
  createNextIssueFromProject(context, content) {
    const projectNameTemplate = handlebars.compile(content.projectName)(context.payload);

    const promise = projects.moveProjectCard(context, content);
    ...
    Promise.resolve(cardsPromise).then(results => {
      const cardId = results[results.length-1].id;
      console.log(cardId);
    });
    //this.createIssue(context, content)
  }
};
