...
module.exports = class Projects extends Plugin {
  ...
  createIssueFromProjectCard(context, content){
    ...
    const issueBodyPromise = Configuration.contents(context.payload.project_card.note);

    const issueBody = Promise.resolve(issueBodyPromise).then(body => handlebars.compile(body)(context.payload));
  }
